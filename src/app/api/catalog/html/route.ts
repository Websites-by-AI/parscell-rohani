...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--paper);color:var(--ink);font-family:Tahoma,Arial,sans-serif;line-height:1.7}.page{max-width:1160px;margin:auto;padding:28px 18px 64px}
    ...
    .contact{display:grid;grid-template-columns:1fr auto;align-items:center;gap:20px;padding:24px;background:linear-gradient(125deg,#fff 50%,#fbeaf1);border-color:#efd1dd}.contact-title{display:flex;align-items:center;gap:12px}.contact-links{display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-end}.ig-icon{display:grid;place-items:center;width:46px;height:46px;border-radius:14px;background:linear-gradient(135deg,#833ab4,#e1306c,#f77737);color:#fff;font-size:21px;font-weight:bold}.contact h3{margin:0;font-size:16px}.contact p{margin:5px 0 0;color:var(--muted);font-size:10px}.ig-link{display:inline-block;background:var(--pink);color:#fff;text-decoration:none;padding:11px 16px;border-radius:11px;font-size:11px;font-weight:900;direction:ltr}.tg-link{display:inline-block;background:#1f94c9;color:#fff;text-decoration:none;padding:11px 16px;border-radius:11px;font-size:11px;font-weight:900;direction:ltr}
    ...
    @media(max-width:760px){.hero{padding:30px 24px}.brand{position:static;text-align:right;margin-bottom:30px}.kpis{grid-template-columns:1fr 1fr}.models{grid-template-columns:1fr}.contact{grid-template-columns:1fr}.contact-links{justify-content:center}.ig-link,.tg-link{text-align:center}.footer{flex-direction:column}}
  ...
  <main class="page">
    ...
    <section class="section card contact">
      ...
      <div class="contact-links">
        ...
        <a class="tg-link" href="https://t.me/Pars_sell_bot" target="_blank" rel="noopener noreferrer">Telegram: @Pars_sell_bot ↗</a>
      </div>
    </section>
